...
fgBool fgGfxPlatform::initialize(fgBool reinit)
{
	...
	if(status) {
		FG_LOG::PrintDebug("GL vendor:    %s", (const char*)glGetString(GL_VENDOR));
		FG_LOG::PrintDebug("GL renderer:  %s", (const char*)glGetString(GL_RENDERER));
		FG_LOG::PrintDebug("GL version:   %s", (const char*)glGetString(GL_VERSION));
		FG_LOG::PrintDebug("GL Shading Lang version: %s", (const char*)glGetString(GL_SHADING_LANGUAGE_VERSION));
		FG_LOG::PrintDebug("Extensions:   %s", (const char*)glGetString(GL_EXTENSIONS));
		if(!m_gfxContext)
