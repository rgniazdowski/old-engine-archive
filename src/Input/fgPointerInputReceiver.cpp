 ...
 * @param point Pointer position
 */
void fgPointerInputReceiver::handlePointerMoved(fgVector2i point, unsigned int touchID)
{
	if(touchID >= MAX_TOUCH_POINTS)
		return;
	
	m_rawTouches[touchID].m_touchID = touchID;
	m_rawTouches[touchID].m_pressed = FG_TRUE;
	m_rawTouches[touchID].m_state = FG_POINTER_STATE_DOWN;
...
int32_t fgPointerInputReceiver::multiTouchMotionHandler(void* systemData, void* userData)
{
	...
	if(!inputReceiver->m_init)
		return 0;
	inputReceiver->handlePointerMoved(fgVector2i(event->m_x, event->m_y), event->m_TouchID);
	return 0;
}
